 ...
 * much better performance thFLASH_PG_BUFFER_SIZERapport de recherche INRIA no 2829, 1996.
 */
...
#define BCH_ECC_WORDS(_p) ((GF_M(_p) * GF_T(_p) + 31) >> 5)
#define BCH_ECC_BYTES(_p) ((GF_M(_p) * GF_T(_p) + 7) >> 3)

#define BCH_ECC_MAX_WORDS ((BCH_MAX_M * BCH_MAX_T + 31) >> 5)
                ...
                uint8_t* ecc)
{
    ...
    if (len) bch_encode_unaligned(bch, data, len, bch->ecc_buf);

    /* store ecc parity bytes into original parity buffer */
    if (ecc) store_ecc8(bch, ecc, bch->ecc_buf);
}
                               ...
                               unsigned int* sol, int nsol)
{
    ...
    if (k > 0) {
        p = k;
        for (r = m - 1; r >= 0; r--) {
            if ((r > m - 1 - k) && rows[r]) /* system has no solution */
                return 0;
    ...
    if (nsol != (1 << k)) /* unexpected number of solutions */
        return 0;
                                ...
                                unsigned int* roots)
{
    ...
    if (poly->c[0]) /* poly[X] = bX+c with c!=0, root=c/b */
                                ...
                                unsigned int* roots)
{
    ...
    if (poly->c[0]) {
        ...
        if (find_affine4_roots(bch, a, b, c, tmp) == 4) {
            ...
            for (i = 0; i < 4; i++) {
                if (tmp[i] != a2) roots[n++] = a_ilog(bch, tmp[i]);
            }
        }
    }
    return n;
                                ...
                                unsigned int* roots)
{
    ...
    if (a) {
        ...
        if (d == 0) /* assume all roots have multiplicity 1 */
            return 0;
                        ...
                        const struct gf_poly* b, int* rep)
{
    ...
    for (j = a->deg; j >= d; j--) {
        if (c[j]) {
            ...
            for (i = 0; i < d; i++, p++) {
                m = rep[i];
                if (m >= 0) c[p] ^= bch->a_pow_tab[mod_s(bch, m + la)];
            }
        }
    }
    a->deg = d - 1;
                                 ...
                                 struct gf_poly* out)
{
    ...
    for (i = 0; i < m; i++) {
        ...
        if (z->deg > out->deg) out->deg = z->deg;
                           ...
                           struct gf_poly* poly, unsigned int* roots)
{
    ...
    default:
        ...
        if (poly->deg && (k <= GF_M(bch))) {
            ...
            if (f1) cnt += find_poly_roots(bch, k + 1, f1, roots);
            if (f2) cnt += find_poly_roots(bch, k + 1, f2, roots + cnt);
        }
        break;
    }
    return cnt;
                        ...
                        struct gf_poly* p, unsigned int* roots)
{
    ...
    for (i = GF_N(bch) - k + 1; i <= GF_N(bch); i++) {
        ...
        for (j = 1, syn = syn0; j <= p->deg; j++) {
            m = bch->cache[j];
            if (m >= 0) syn ^= a_pow(bch, m + j * i);
        }
        if (syn == 0) {
            roots[count++] = GF_N(bch) - i;
            if (count == p->deg) break;
        }
    }
               ...
               const unsigned int* syn, unsigned int* errloc)
{
    ...
    if (8 * len > (bch->n - bch->ecc_bits)) return -EINVAL;

    /* if caller does not provide syndromes, compute them */
    if (!syn) {
        if (!calc_ecc) {
            /* compute received data ecc into an internal buffer */
            if (!data || !recv_ecc) return -EINVAL;
...
static void build_mod8_tables(struct bch_control* bch, const uint32_t* g)
{
    ...
    const int plen = (bch->ecc_bits + 32) >> 5;
    const int ecclen = (bch->ecc_bits + 31) >> 5;
...
static uint32_t* compute_generator_polynomial(struct bch_control* bch)
{
    ...
    genpoly = bch_alloc(((m * t + 32) >> 5) * sizeof(*genpoly), &err);
                             ...
                             int swap_bits)
{
    ...
    /* sanity checks */
    if ((t < 1) || (m * t >= ((1 << m) - 1))) /* invalid t value */
        goto fail;

    /* select a primitive polynomial for generating GF(2^m) */
    if (prim_poly == 0) prim_poly = prim_poly_tab[m - min_m];

    bch = malloc(sizeof(*bch));
    if (bch == NULL) goto fail;
